...
files = {
    r"lib\features\marketplace\presentation\providers\bulk_requirement_provider.dart": """
...
""",
    r"lib\features\marketplace\presentation\providers\bulk_offer_provider.dart": """
import 'package:flutter/foundation.dart';
...
class BulkOfferProvider with ChangeNotifier {
  ...
  Future<void> acceptOffer(String offerId, double acceptedQuantity) async {
    ...
    try {
      ...
      final req = await _requirementRepository.getRequirementById(offer.requirementId);
      if (req.status != BulkRequirementStatus.open && req.status != BulkRequirementStatus.partiallyFulfilled) {
      ...
      // Proceed to accept
      await _offerRepository.updateOfferStatus(offerId, BulkOfferStatus.accepted, acceptedQuantity: acceptedQuantity);
      ...
      final index = _offers.indexWhere((o) => o.offerId == offerId);
      if (index >= 0) {
        _offers[index] = _offers[index].copyWith(status: BulkOfferStatus.accepted, acceptedQuantity: acceptedQuantity);
      }
    } catch (e) {
      _error = e.toString();
      rethrow;
  ...
  Future<void> rejectOffer(String offerId) async {
    ...
    try {
      ...
      await _offerRepository.updateOfferStatus(offerId, BulkOfferStatus.rejected);
      
      final index = _offers.indexWhere((o) => o.offerId == offerId);
      if (index >= 0) {
        _offers[index] = _offers[index].copyWith(status: BulkOfferStatus.rejected);
      }
    } catch (e) {
      _error = e.toString();
      rethrow;
